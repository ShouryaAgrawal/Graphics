...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    let ohrs_geom = new T.BoxGeometry(0.5,0.5,0.5);
    let ohrs_mat = new T.MeshStandardMaterial({
      color: "#aaaaaa",
      metalness: 0.8,
      roughness: 0.5
    });
    ...
    for (let i = 0; i < num_poles; i++) {
      ...
      ohrs = new T.Mesh(ohrs_geom, ohrs_mat);
   ...
   */
   stepWorld(delta, timeOfDay) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      //console.log(Math.sin(delta)/10);
     //this.ohrss[i].position.y = 0.3 + Math.sin(delta)/5;


     this.time += delta / 10000; // time in seconds
     // set the y position based on the time
     //let t =0.1 // where are we in the cycle?
     
     if (this.ohrss[i].position.y  <= 0.1 ) {
       this.ohrss[i].position.y = 0.11;
       this.fl=0;
     //  console.log("haha");
    }
     else if (this.ohrss[i].position.y  >= 0.6) {
      this.ohrss[i].position.y = 0.59;
      this.fl =1;
     // console.log("haha22");
    }
     else {
      if (this.fl ==0){   
        this.ohrss[i].position.y += 0.05;
      }
      else if (this.fl==1) { 
        this.ohrss[i].position.y -= 0.05 ;
    ...
    this.whole_ob.rotateY(0.0005 * delta);
  }
